    ...
    <>
      ...
      <section className="section">
        ...
        <p style={{ fontSize: "0.85rem", color: "var(--color-text-muted)", marginBottom: "0.5rem" }}>
          ...
          The test that separates the last state from the others is per-line and reproducible: in a SAGE clone, count the commits under <code>sage/instances/&lt;line&gt;/</code> whose subject <em>begins</em> <code>raising: &lt;line&gt; autonomous session</code> and that land <em>after</em> the line&apos;s most recent session record. Match the prefix, not the whole subject &mdash; the real commit subject carries a trailing ISO timestamp, so an exact-match reader gets 0 on every line including HUB, and the test fails to reproduce its own result. HUB scores 165. Every other line on this page scores 0.
          The keying is load-bearing, and this page publishes it because the looser version is wrong: scored on <em>any</em> commit touching the instance directory, <code>legion-gemma4-e4b</code> reads 366 and outranks HUB, because a line&apos;s <code>peer_trust_rs.json</code> moves for reasons that are not that line running. Counting the line&apos;s own raising commits clears the field.
        ...
        <div className="grid-3">
          ...
          <MachineCard
            ...
            liveness={
              ...
              "So the shape is HUB's, at a second site and with an earlier start — a loop that fires on schedule, emits one pre-session artifact and completes nothing — except that E4B's stopped on 2026-08-27 while HUB's is still firing today. Why it fired at all, and why it stopped, is Legion's seat to answer; the record establishes that it is not an unprovisioned instance."
            }
