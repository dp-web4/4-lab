    ...
    <>
      ...
      <section className="section">
        ...
        <div className="grid-2">
          ...
          <ProjectCard
            name="Hub"
            description="Rust society runtime — a single ~6 MB binary that turns a community or organization into a sovereign Web4 society: member identity, seven base roles, a signed founding charter, an append-only hash-chained witnessed ledger, a full REST API with a small MCP (Model Context Protocol) tool surface beside it, sealed member channels, and an admin web GUI and CLI. Where Hestia governs one machine, Hub is the society the governed members join; Hestia makes the record, Hub is what checks it. The public repo is, in upstream's words, the open reference proof-of-concept: production and advanced development continue in a separate private repository, and features may land in the open crate later. MVP complete through sprint 6 and buildable, with post-MVP work (role entities, a plugin seam, quorum release of protected items) ongoing in the open repo. It is pilot-stage rather than deployed: the Docker package has not been tested at all, not even on the development machine (the README asks the first operator who runs it to report), and no chapter has run it in production use. The fleet runs on the same society runtime it is developing — the eight machines are members of a society with a chapter ledger, which is why /fleet can say the society-hosts are members of what they host."
            accentColor="#14b8a6"
            isPublic={true}
            maturity="runnable"
          ...
          <ProjectCard
            ...
            maturity="research-stage"
            links={[
              { label: "OpenClaw", url: "https://github.com/dp-web4/openclaw/tree/web4-governance-complete/extensions/web4-governance" },
...
# start there and in docs/, and expect to read Go/Cosmos source.`}</pre>
        <p style={{ fontSize: "0.8125rem", color: "var(--color-text-muted)", marginBottom: "0.75rem" }}>
          ...
          consciousness kernel&rdquo; (an upstream code name that predates this site&apos;s framing;
          a legacy label, like the Governance in SAGE&apos;s name, and not a claim). The dashboard at port 8760 shows version, uptime, the model,
